  ...
  cors(req, res, async () => {
    try {
      ...
      if (filters.bookingDateStart && filters.bookingDateEnd) {
        const startDate = new Date(filters.bookingDateStart);
        const endDate = new Date(filters.bookingDateEnd);
        const dateRange = [];
        ...
        query = query.where("appointmentDate", "in", dateRange);
      } else if (filters.bookingDateStart) {
        query = query.where("appointmentDate", ">=", filters.bookingDateStart);
      } else if (filters.bookingDateEnd) {
      ...
      if (filters.createDateStart || filters.createDateEnd) {
      ...
      if (sortCriteria && sortCriteria.key) {
        if (sortCriteria.key !== "createdAt") {
          query = query.orderBy(sortCriteria.key, sortCriteria.order);
        }
        if (sortCriteria.key !== "createdAt" || sortCriteria.order !== "desc") {
      ...
      // Server-side filtering for notes
      if (filters.notes) {
